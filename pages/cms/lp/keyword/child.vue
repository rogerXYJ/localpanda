      ...
      <div class="keyword-table-box" v-show="tableShow">
        <el-table
          ...
          <el-table-column label="操作" width="180">
            <template slot-scope="scope">
              <a class="btn_text" :href="'/cms/lp/keyword/tpl'+scope.row.level+'?id='+scope.row.id">编辑</a>
...
// import binding from '~/components/cms/lp/binding.vue';
...
export default {
  ...
  methods:{
    ...
    removeBind(row,index){
      ...
      this.axios.delete('https://api.localpanda.com/api/content/landingpage/binding/delete/'+row.binding.id,{
        headers: {
    ...
    getChildList(){
      ...
      //请求基本信息
      this.axios.get('https://api.localpanda.com/api/content/landingpage/info/'+this.pageId).then(function(response) {
        if(response.status == 200){
