...
<script>
	...
	export default {
		...
		methods: {
					...
					token: function(token) { // 用户填写完资料并且 Stripe 校验成功后的回调函数
						...
						that.axios.post("https://www.localpanda.com/api/payment/pay/stripe", JSON.stringify(obj), {
							...
							if(response.data.succeed) {
								that.loadingStatus = true
								window.location.href = "https://www.localpanda.com/success.html?orderId=" + that.orderId + '&amount=' + that.opctions.amount
							}
							//
						}, function(response) {})
					}
			...
			getInfo(){
				let that=this
				that.axios.get("https://www.localpanda.com/api/activity/order/detail/"+that.orderId).then(function(res){
