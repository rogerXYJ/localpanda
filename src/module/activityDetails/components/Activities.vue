			...
			<div class="activitiesInfo">
				...
				<ul class="description clearfix">
					...
					<li>
						...
						<span v-if="picInfo.minParticipants==picInfo.maxParticipants">{{picInfo.minParticipants}}</span>
				...
				<div class="heightLights" id="heightLights">
					<p class="clearfix" v-for="item in highlights"><i class="iconfont">&#xe654;</i><span>{{item}}</span></p>
				...
				<div class="notes" id="notes">
					<h3>Notes</h3>
					<p v-if="remark" v-for="item in remark">{{item}}</p>
					...
					<p v-if="detail.venues" v-for="i in detail.venues">{{i}}</p>
				</div>
									...
									<div class="selectTime" v-if="picInfo.departureTime">
										...
										<div class="time" @click.stop="showTime">
											...
											<div v-if="isShowTime" @click.stop="showTime" class="timeList">
												<ul>
													<li v-for="(item,index) in picInfo.departureTime" @click.stop="confirmTime(index)">{{item}}</li>
...
<script>
	import { GetDateStr, addmulMonth, getUrlParams } from "../../../assets/js/plugin/utils"
	import Contact from '../../guideDetail/components/Contact';
	import Alert from '../../../components/Prompt/Alert';
	import flatPickr from 'vue-flatpickr-component';
	import 'flatpickr/dist/flatpickr.css';
	export default {
		props: ["remark", "detail", "highlights", "destinations", "itemsIncluded", "id", "isscroll", "isShowBookNow", "introduction"],
		name: 'Activities',
		data() {
			return {
				open: null,
			...
			order() {
				...
				if(that.dateTime == "") {
					...
					var orderInfo = {
						...
						"amount": that.children > 0 && that.picInfo.childDiscount ? that.cutXiaoNum(that.adultsPic - that.children * that.picInfo.childDiscount, 1) : that.adultsPic,
						"currency": "USD",
						...
						"averagePrice": that.round(that.adultsPic / (that.adults + that.children)),
						"childDiscount": that.picInfo.childDiscount ? that.children * that.picInfo.childDiscount : null
		...
		mounted: function() {
			let that = this
			that.getRecommend()
			that.axios.get("https://www.localpanda.com/api/activity/price/" + that.id).then(function(res) {
